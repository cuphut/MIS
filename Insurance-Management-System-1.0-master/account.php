            ...
            <div class="row">
                <div class="col-lg-8 posts-list">
                    <form action="change_password.php" method="post">
                        <h1 style="color: black">User Account Details
                            <hr style="color: black">
                        </h1>
                        <p style="color: black">This is your account details.</p>
                        
                        <?php
                            $query = "SELECT `user_id` FROM `users` WHERE `username`='$username'";
                            $results = mysqli_query($conn, $query);
                            while ($row = $results->fetch_assoc()) {
                                echo "<p class='fs-5' style='color: black'>User Id: ".$row['user_id']."</p>";
                            }

                            echo "<p class='fs-5' style='color: black'>Username: ".$username."</p>";

                            $query = "SELECT `password` FROM `users` WHERE `username`='$username'";
                            $results = mysqli_query($conn, $query);
                            while ($row = $results->fetch_assoc()) {
                                echo "<p class='fs-5' style='color: black'>Password: ".$row['password']."</p>";
                            }

                            $query = "SELECT `email` FROM `users` WHERE `username`='$username'";
                            $results = mysqli_query($conn, $query);
                            while ($row = $results->fetch_assoc()) {
                                echo "<p class='fs-5' style='color: black'>Email: ".$row['email']."</p>";
                            }
                            ?>
                        <br>
